...
router.get("/getVideos", async (req, res) => {
  ...
  try {
    let response = limit
      ? await VideoModal.find(filter).limit(limit)
      : await VideoModal.find(filter);
    if (response.length) {
      res.status(200).json({
...
router.get("/getVideos/:videoId", async (req, res) => {
  ...
  let filter = { _id: videoId };
  try {
    const video = await VideoModal.findOne(filter);
    let update_content = { views: Number(video.views) + 1 };
    let response = await VideoModal.findByIdAndUpdate(filter, update_content, {
      new: true,
    });
    res.status(200).json({
